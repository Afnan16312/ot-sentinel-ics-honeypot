...
| F-52 | Explainable public investigation workflow | A public dashboard needs to help a reviewer interpret evidence without revealing private telemetry or claiming a source is malicious. | The workspace explains terms and provenance, compares up to three aggregate pseudonymous map observations, uses adjacent non-overlapping windows for cautious change summaries, and displays a public-safe review score, score factors, and a non-automated next step for the selected observation. | `app.py`, `dashboard_map.py`, `triage.py`, dashboard/triage tests, `UX_PRODUCT_ROADMAP_2026.md` | The public score deliberately excludes private repeat/novelty evidence; counts, map points, and recommendations are not attribution, proof of intrusion, or automated response. |
