...
This catalog explains what OT Sentinel ships, why each feature exists, how it works, where its evidence lives, and what it does **not** prove. It describes release `v0.2.0`.
...
| F-14 | Explicit provenance labels | Demonstration data must not be mistaken for captured attacks. | Events and reports distinguish `synthetic`, `simulated`, and authorized `live` origins. | Data model, report generator, public-data checks | This release contains no claimed UAE live-attacker dataset. |
...
| F-30 | Local, Docker, service, and cloud templates | The project should work free locally and remain deployable later. | Runbooks and configuration cover local execution, hardened Docker, systemd, and optional Azure infrastructure. | `Dockerfile`, `docker-compose.yml`, `infra/`, `docs/DEPLOYMENT.md` | Cloud resources can cost money and are not required for the portfolio demonstration. |
