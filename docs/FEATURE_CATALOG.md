...
| F-39 | Disposable native SOC lab assets | Static checks cannot prove Wazuh/Suricata runtime behavior. | A pinned loopback-only Compose harness, synthetic injector and PCAP verifier define authoritative positive/negative checks. | `tests/soc/`, harness tests | Docker is unavailable locally, so native engine validation is not claimed yet. |
