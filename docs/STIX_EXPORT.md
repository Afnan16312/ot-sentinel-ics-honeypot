...
The salt is needed only when the input still contains `source_ip`. Already-sanitized events containing `source_id` can be exported without it.
...
- Public-profile safety is enforced during export. A Bundle is rejected if a payload field or a standalone IP-address literal survives serialization.
