...
| Native SOC lab | Disposable, loopback-only Wazuh 4.14.7 and Suricata 8.0.4 harness, deterministic PCAP and positive/negative injectors | Provide an authoritative destination-engine path rather than relying only on static checks | harness/static tests and deterministic fixture validation | Docker is unavailable in this development environment, so native engine outputs are not claimed |
...
Native Wazuh and Suricata validation needs a separate local Docker host with at least 4 CPU, 8 GB RAM and 50 GB free disk. The final video needs those reviewed native outputs plus a green feature-branch GitHub Actions run. Neither gap changes the local software or authorizes a cloud deployment.

## Local verification on 2026-08-25

- Complete pytest suite: **149 passed, 10 subtests passed**.
- Ruff: passed.
...
- Native Wazuh/Suricata: **not run** because the Docker CLI is not installed. The host otherwise reports 15.7 GB RAM and sufficient disk, but installing Docker is a separate system-level human action.
