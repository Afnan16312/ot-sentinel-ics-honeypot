...
- Added a guided investigation path that explains Scope, Prioritize and Validate, then prepares the strongest evidence-backed source for Session Explorer or ATT&CK review without making an automated decision.
- Added an actionable detection coverage backlog that connects selected behavior to mapping, offline rule engines, synthetic fixture evidence, and the next engineering action.
- Added an opt-in, read-only operator assurance panel for allowlisted aggregate local health counters.
